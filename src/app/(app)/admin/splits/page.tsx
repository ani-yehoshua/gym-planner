...
const stepBtn =
    "rounded-md border border-border px-2 py-1 text-xs disabled:opacity-30";
        ...
        <div className='flex flex-col gap-2'>
            ...
            {(templates ?? []).map(t => {
                ...
                const dayByPosition = new Map(
                    t.template_days.map(d => [d.position, d]),
                );
                        ...
                        <div className='flex flex-col gap-4 border-t border-border p-3'>
                            ...
                            {slots
                                .map((s, i) => ({
                                    s,
                                    i,
                                    day: dayByPosition.get(i),
                                }))
                                .filter(x => x.s !== "rest" && x.day)
                                .map(({ s, i, day }) => {
                                    const exs = [
                                        ...day!.template_day_exercises,
                                    ].sort((a, b) => a.sort - b.sort);
                                    const pool = cat.filter(c =>
                                        DAY_ACCEPTS[s].includes(c.category),
                                    );
                                    return (
                                        <div
                                            key={day!.id}
